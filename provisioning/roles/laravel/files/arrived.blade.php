<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8">
	    <meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1">
	    <title>Laravel Homestead Bootstrap LAMP Stack</title>

	    <!-- Bootstrap -->
	    <link href="//cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.2.0/css/bootstrap.min.css" rel="stylesheet">

	    <!-- HTML5 Shim and Respond.js IE8 support of HTML5 elements and media queries -->
	    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
	    <!--[if lt IE 9]>
	      <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
	      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
	    <![endif]-->
		<style>
			@import url(//fonts.googleapis.com/css?family=Lato:700);

			body {
				font-family:'Lato', sans-serif;
				color: #999;
			}
			#logo {
				float:right;
				padding:6px 0 6px 0;
			}

			h1 {
				font-size: 32px;
				margin: 16px 0 0 0;
			}
			.lead {
				padding:6px 0 6px 0;
			}
		</style>
	</head>
	<body>
		<div class="container">
			<div id="logo">
				<a href="http://laravel.com" title="Laravel PHP Framework"><img src="data:image/png;base64," alt="Laravel PHP Framework"></a>
			</div>

			<h1>Laravel 5 Homestead Bootstrap</h1>
			<p class="lead">
				The virtual machine ({{ Request::server('SERVER_SOFTWARE') }}) is up and running. Hot Damn!
			</p>
			<p class="lead">
				Here be some additional information you might need:
			</p>
			<h2>Installed software</h2>
			<table class="table table-striped">
		        <tr>
					<td>PHP Version</td>
					<td>{{ $phpversion }}</td>
		        </tr>
		        <tr>
					<td>MySQL running</td>
					<td><span class="glyphicon glyphicon-{{ $mysql_running ? 'ok' : 'remove' }}"></span></td>
		        </tr>
		        <tr>
					<td>MySQL version</td>
					<td>{{ $mysql_version }}</td>
		        </tr>
		        <tr>
					<td>Memcached running</td>
					<td><span class="glyphicon glyphicon-{{ $memcached_running ? 'ok' : 'remove' }}"></span></td>
		        </tr>
		        <tr>
					<td>Memcached version</td>
					<td>{{ $memcached_version }}</td>
		        </tr>
			</table>

			<h2>PHP Modules</h2>
			<table class="table table-striped">
				<tr>
					<td>MySQL</td>
					<td><span class="glyphicon glyphicon-<?= class_exists('mysqli') ? 'ok' : 'remove' ?>"></span></td>
		        </tr>

		        <tr>
					<td>cURL</td>
					<td><span class="glyphicon glyphicon-<?= function_exists('curl_init') ? 'ok' : 'remove' ?>"></span></td>
		        </tr>

		        <tr>
					<td>mcrypt</td>
					<td><span class="glyphicon glyphicon-<?= function_exists('mcrypt_encrypt') ? 'ok' : 'remove' ?>"></span></td>
		        </tr>

		        <tr>
					<td>memcached</td>
					<td><span class="glyphicon glyphicon-<?= class_exists('Memcached') ? 'ok' : 'remove' ?>"></span></td>
		        </tr>

		        <tr>
					<td>gd</td>
					<td><span class="glyphicon glyphicon-<?= function_exists('imagecreate') ? 'ok' : 'remove' ?>"></span></td>
		        </tr>
			</table>

			<h2>MySQL credentials</h2>
			<table class="table table-striped">
		        <tr>
					<td>Hostname</td>
					<td>{{ $mysql_host }}</td>
		        </tr>
		        <tr>
					<td>Username</td>
					<td>{{ $mysql_username }}</td>
		        </tr>
		        <tr>
					<td>Password</td>
					<td>{{ $mysql_password }}</td>
		        </tr>
			</table>

			<h2>NPM Packages</h2>
			<table class="table table-striped">
@foreach($npm_packages['devDependencies'] as $pkg => $ver)
				<tr>
					<td>{{ $pkg }}</td>
					<td>{{ $ver }}</td>
		        </tr>
@endforeach
			</table>
			<p>
				<em>Note: External access is enabled! Just use <a href="http://{{ Request::server('SERVER_ADDR') }}" title="Your server's address">{{ Request::server('SERVER_ADDR') }}</a> as host.</em>
			</p>
		</div>

	    <!-- jQuery (necessary for Bootstrap's JavaScript plugins) -->
	    <script src="//cdnjs.cloudflare.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
	    <!-- Include all compiled plugins (below), or include individual files as needed -->
	    <script src="//cdnjs.cloudflare.com/ajax/libs/twitter-bootstrap/3.2.0/js/bootstrap.min.js"></script>
	</body>
</html>